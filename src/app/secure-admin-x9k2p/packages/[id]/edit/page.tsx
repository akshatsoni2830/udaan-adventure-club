'use client';

import { useState, useEffect } from 'react';
import { useRouter, useParams } from 'next/navigation';
import Link from 'next/link';

export default function EditPackage() {
  const router = useRouter();
  const params = useParams();
  const id = params.id as string;
  
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [formData, setFormData] = useState({
    title: '',
    duration: '',
    departureCities: '',
    fixedDepartures: '',
    costDetails: '',
    inclusions: '',
    notes: '',
    itemsToCarry: '',
    paymentTerms: '',
    cancellationTerms: '',
    images: ''
  });

  useEffect(() => {
    fetchPackage();
  }, [id]);

  const fetchPackage = async () => {
    try {
      const response = await fetch(`/api/packages/${id}`);
      if (response.ok) {
        const data = await response.json();
        const pkg = data.package;
        
        setFormData({
          title: pkg.title || '',
          duration: pkg.duration || '',
          departureCities: Array.isArray(pkg.departureCities) ? pkg.departureCities.join(', ') : pkg.departureCities || '',
          fixedDepartures: Array.isArray(pkg.fixedDepartures) ? pkg.fixedDepartures.join(', ') : pkg.fixedDepartures || '',
          costDetails: Array.isArray(pkg.costDetails) 
            ? pkg.costDetails.map((c: { description: string; price: string }) => `${c.description}: ${c.price}`).join('; ')
            : pkg.costDetails || '',
          inclusions: Array.isArray(pkg.inclusions) ? pkg.inclusions.join('; ') : pkg.inclusions || '',
          notes: Array.isArray(pkg.notes) ? pkg.notes.join('; ') : pkg.notes || '',
          itemsToCarry: Array.isArray(pkg.itemsToCarry) ? pkg.itemsToCarry.join('; ') : pkg.itemsToCarry || '',
          paymentTerms: Array.isArray(pkg.paymentTerms) ? pkg.paymentTerms.join('; ') : pkg.paymentTerms || '',
          cancellationTerms: Array.isArray(pkg.cancellationTerms) ? pkg.cancellationTerms.join('; ') : pkg.cancellationTerms || '',
          images: Array.isArray(pkg.images) ? pkg.images.join(', ') : pkg.images || ''
        });
      } else {
        alert('Failed to load package');
        router.push('/secure-admin-x9k2p/dashboard');
      }
    } catch (error) {
      console.error('Error fetching package:', error);
      alert('Error loading package');
    } finally {
      setLoading(false);
    }
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);

    try {
      const response = await fetch(`/api/admin/packages/${id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify(formData)
      });

      if (response.ok) {
        alert('Package updated successfully!');
        router.push('/secure-admin-x9k2p/dashboard');
      } else {
        const data = await response.json();
        alert(`Failed to update package: ${data.error || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Error updating package:', error);
      alert('Error updating package');
    } finally {
      setSaving(false);
    }
  };

  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center ">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600 mx-auto"></div>
          <p className="mt-4 text-gray-600">Loading package...</p>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gray-100 text-black">
      <header className="bg-white shadow">
        <div className="max-w-7xl mx-auto px-4 py-4 sm:px-6 lg:px-8 flex justify-between items-center">
          <h1 className="text-2xl font-bold text-gray-900">Edit Package</h1>
          <Link
            href="/secure-admin-x9k2p/dashboard"
            className="px-4 py-2 bg-gray-600 text-white rounded hover:bg-gray-700 transition-colors"
          >
            Back to Dashboard
          </Link>
        </div>
      </header>

      <main className="max-w-4xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
        <div className="bg-white rounded-lg shadow p-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Package Title *
              </label>
              <input
                type="text"
                value={formData.title}
                onChange={(e) => setFormData({ ...formData, title: e.target.value })}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                required
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Duration *
              </label>
              <input
                type="text"
                value={formData.duration}
                onChange={(e) => setFormData({ ...formData, duration: e.target.value })}
                placeholder="e.g., 3 Days 2 Nights"
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                required
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Departure Cities (comma-separated)
              </label>
              <input
                type="text"
                value={formData.departureCities}
                onChange={(e) => setFormData({ ...formData, departureCities: e.target.value })}
                placeholder="e.g., Ahmedabad, Mumbai, Delhi"
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Fixed Departures (comma-separated)
              </label>
              <input
                type="text"
                value={formData.fixedDepartures}
                onChange={(e) => setFormData({ ...formData, fixedDepartures: e.target.value })}
                placeholder="e.g., 15th Jan, 20th Feb"
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Cost Details (semicolon-separated)
              </label>
              <textarea
                value={formData.costDetails}
                onChange={(e) => setFormData({ ...formData, costDetails: e.target.value })}
                placeholder="e.g., Per Person: ₹15,000; Group of 4: ₹50,000"
                rows={3}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Inclusions (semicolon-separated)
              </label>
              <textarea
                value={formData.inclusions}
                onChange={(e) => setFormData({ ...formData, inclusions: e.target.value })}
                placeholder="e.g., Accommodation; Meals; Transport"
                rows={4}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Notes (semicolon-separated)
              </label>
              <textarea
                value={formData.notes}
                onChange={(e) => setFormData({ ...formData, notes: e.target.value })}
                placeholder="e.g., Minimum 4 people required; Advance booking recommended"
                rows={3}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Items to Carry (semicolon-separated)
              </label>
              <textarea
                value={formData.itemsToCarry}
                onChange={(e) => setFormData({ ...formData, itemsToCarry: e.target.value })}
                placeholder="e.g., Comfortable shoes; Sunscreen; Water bottle"
                rows={3}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Payment Terms (semicolon-separated)
              </label>
              <textarea
                value={formData.paymentTerms}
                onChange={(e) => setFormData({ ...formData, paymentTerms: e.target.value })}
                placeholder="e.g., 50% advance required; Balance before departure"
                rows={3}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Cancellation Terms (semicolon-separated)
              </label>
              <textarea
                value={formData.cancellationTerms}
                onChange={(e) => setFormData({ ...formData, cancellationTerms: e.target.value })}
                placeholder="e.g., 100% refund if cancelled 15 days before; 50% refund if cancelled 7 days before"
                rows={3}
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              />
            </div>

            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
                Images (comma-separated paths) *
              </label>
              <input
                type="text"
                value={formData.images}
                onChange={(e) => setFormData({ ...formData, images: e.target.value })}
                placeholder="e.g., /ll1.jpg, /ll2.jpg, /ll3.jpg"
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                required
              />
              <p className="text-xs text-gray-500 mt-1">
                Enter image paths from the /public folder
              </p>
            </div>

            <div className="flex gap-4">
              <button
                type="submit"
                disabled={saving}
                className="flex-1 px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed font-medium"
              >
                {saving ? 'Saving...' : 'Save Changes'}
              </button>
              <Link
                href="/secure-admin-x9k2p/dashboard"
                className="px-6 py-3 bg-gray-300 text-gray-700 rounded-lg hover:bg-gray-400 transition-colors font-medium text-center"
              >
                Cancel
              </Link>
            </div>
          </form>
        </div>
      </main>
    </div>
  );
}
